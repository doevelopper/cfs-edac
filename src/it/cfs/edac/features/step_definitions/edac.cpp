...
namespace {
    ...
    std::string get_features_directory(const char *executable, std::string feature_directory)
    {
...
int main(int argc, char **argv)
{
    ...
    std::thread client([argc, argv, &client_result]()
        {
                       std::string features_directory = get_features_directory(argv[0], argv[1]);
                       std::string command
                       {
                           build_cucumber_command(argc - 1, argv + 1, features_directory)
                       };
