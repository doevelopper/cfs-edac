...
#ifndef CFS_UTILS_NAMEDTYPE_UNDERLYINGFUNCTIONALITIES_HPP
#define CFS_UTILS_NAMEDTYPE_UNDERLYINGFUNCTIONALITIES_HPP

#include <functional>
#include <iostream>
#include <memory>

#include <cfs/utils/namedType/Crtp.hpp>
#include <cfs/utils/namedType/internal/NamedTypeInternal.hpp>

namespace cfs::utils::namedType
{
    template <typename T>
    ...
    std::ostream& operator<<(std::ostream& os, cfs::utils::namedType::NamedType<T, Parameter, Skills...> const& object)
    {
        object.print(os);
    ...
    {
        template <typename T, typename Parameter, template<typename> class ... Skills>
        struct hash<cfs::utils::namedType::NamedType<T, Parameter, Skills...> >
        {
            using NamedType = cfs::utils::namedType::NamedType<T, Parameter, Skills...>;
            using checkIfHashable = typename std::enable_if<NamedType::is_hashable, void>::type;
            size_t operator()(cfs::utils::namedType::NamedType<T, Parameter, Skills...> const& x) const
            {
